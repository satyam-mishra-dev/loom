...
describe('SIGNATURE (crash variant): kill the matcher mid-cascade', () => {
  ...
  function spawnService(script: string, env: Record<string, string>): ChildProcess {
    ...
    child.stdout?.on('data', (chunk: Buffer) =>
      process.stdout.write(`[${script}] ${String(chunk)}`),
    );
  ...
  it('claims released, OFFERED trips re-enqueued, restarted matcher completes — invariant never violated', async () => {
    ...
    try {
      ...
      await waitFor(
        async () => (await redis.keys('claim:*')).length === CONSUMERS,
        30_000,
        'claims held',
      );
      ...
      expect(
        (await pool.query(`SELECT count(*)::int AS n FROM trips WHERE status = 'matching'`))
          .rows[0],
      ).toEqual({ n: CONSUMERS });
      expect(
        (await pool.query(`SELECT count(*)::int AS n FROM ride_requests WHERE status = 'pending'`))
          .rows[0],
      ).toEqual({ n: REQUESTS });
