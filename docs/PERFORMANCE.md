...
**Headline:** the match _service_ path is already tight (~12 ms p50 to a
...
- **Co-resident-load caveat (important).** These runs share a laptop with 2–3
  other Docker compose stacks (Tally, a live Loom stack) _plus_ the bench's
  own testcontainers Postgres+Redis. Absolute latencies carry real co-resident
  noise — treat them as a magnitude, and trust the _relative_ before/after
...
1. **Service is tight, queue wait dominates.** `to_matched_total` p50 is
   **11.8 ms** while the end-to-end bench p50 at RPS 100 is **~120–137 ms**. The
   ...
   moves it _indirectly_, by freeing consumers and cutting shared-Postgres load.
2. **The two Postgres transactions are the hottest deterministic phases.**
   ...
   source discussed under _Remaining bottleneck_.

The optimizations therefore target Postgres round trips (which both lowers
service time _and_ reduces load on the shared Postgres), not Redis and not the
...
_File:_ `apps/matcher/src/trip-store.ts`.
...
_File:_ `apps/matcher/src/matcher.ts`.
...
_first_ (before the baseline triplet), so any co-resident drift over the session
...
This is why the kept optimizations go the _other_ direction (fewer round trips,
...
Postgres statements _within the existing transactions and guards_, never across
them.
...
_single multiplexed simulator socket_ (one WS connection standing in for 5 000
...
1. **Cut the offer-stall tail** — this is the real variance lever, worth far
   ...
   _first_ offer deadline with fast re-offer so one dead driver doesn't cost a
